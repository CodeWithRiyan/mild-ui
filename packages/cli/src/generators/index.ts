...
export async function generateComponent(options: GenerateOptions): Promise<void> {
  ...
  const componentDir = path.join(outputDir, component.charAt(0).toUpperCase() + component.slice(1));
  await fs.ensureDir(componentDir);
  
  // Write files
  for (const [filename, content] of Object.entries(templates)) {
    await fs.writeFile(path.join(componentDir, filename), content as 'string | ArrayBufferView<ArrayBufferLike>');
  }

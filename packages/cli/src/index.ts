...
console.log(
  chalk.blue(figlet.textSync("mild-ui CLI", { horizontalLayout: "full" })),
);
...
program
  ...
  .action((component, options) => {
    if (!component && !options.all) {
      console.error(
        chalk.red("Error: Either specify a component name or use --all flag"),
      );
      console.log();
...
program
  ...
  .action((options) => {
    ...
    if (options.all) {
      ...
      console.log(
        chalk.yellow(
          'Initialization complete! Use "mild-ui add [component]" or "mild-ui add --all" to add components.',
        ),
      );
    }
  });
...
if (!process.argv.slice(2).length) {
  ...
  console.log("Examples:");
  console.log(
    "  mild-ui add Button                    # Add Button component (React by default)",
  );
  console.log(
    "  mild-ui add Input --framework=vue     # Add Input component for Vue",
  );
  console.log(
  ...
  console.log(
    "  mild-ui transform Button --from=react --to=vue  # Transform component",
  );
}

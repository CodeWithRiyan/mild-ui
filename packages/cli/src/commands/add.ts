...
import {
  readComponentFromFramework,
  getAvailableComponents,
} from "../utils/component-reader";
...
async function addSingleComponent(options: {
  ...
  const { component, framework, directory, customImportPaths, skipExisting } =
    options;
  ...
  if (fs.existsSync(targetDir)) {
    if (skipExisting) {
      console.log(
        chalk.yellow(`Component ${component} already exists. Skipped.`),
      );
      return;
    } else {
      console.log(
        chalk.yellow(`Component ${component} already exists. Overwriting...`),
      );
    }
  }
  ...
  console.log(
    chalk.cyan(`import { ${component} } from './${directory}/${component}';`),
  );
}
...
async function installComponent(
  ...
  const frameworkComponent = await readComponentFromFramework(
    componentName,
    framework,
  );
...
function printInstallationSummary(
  ...
  if (results.success.length > 0) {
    console.log(
      chalk.green(`✓ Successfully installed (${results.success.length}):`),
    );
  ...
  console.log(
    chalk.cyan(
      `Total components processed: ${results.success.length + results.skipped.length + results.failed.length}`,
    ),
  );
  ...
  if (results.success.length > 0) {
    ...
    console.log(
      `Import them from './${path.basename(process.cwd())}/components/[ComponentName]'`,
    );

    // Show common dependencies
    console.log();
    console.log(
      chalk.yellow("Make sure you have the required dependencies installed:"),
    );

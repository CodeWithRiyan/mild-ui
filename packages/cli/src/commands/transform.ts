...
import {
  ...
  transformComponent,
} from "../utils/component-processor";
...
export async function transformCommand(
  ...
  try {
    ...
    console.log(
      chalk.blue(
        `Transforming ${component} from ${fromFramework} to ${toFramework}...`,
      ),
    );
    ...
    const targetComponent = await readComponentFromFramework(
      component,
      toFramework,
    );
    ...
    console.log();
    console.log(
      chalk.green(`✓ Successfully transformed ${component} to ${toFramework}!`),
    );
    console.log();

...
`;
    ...
    for (const component of components) {
      ...
      if (stat.isDirectory()) {
        const componentInfo = await extractBasicComponentInfo(componentPath, component);
        if (componentInfo) {
...
async function createPlaceholderDocs() {
  await fs.mkdir('docs/generated', { recursive: true });
  await fs.writeFile('docs/generated/component-api.md', `# Component API
...
async function extractBasicComponentInfo(componentPath, componentName) {
  ...
  try {
    ...
    const interfaceRegex = new RegExp(`export interface ${componentName}CoreProps[^{]*{([^}]*)}`, 'gs');
    ...
    const typeRegex = new RegExp(`export type ${componentName}\\w* = ([^;]+);`, 'g');
    let typeMatch;

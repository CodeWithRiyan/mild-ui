...
export interface ClassObject extends Record<string, boolean | undefined | null> {}
  ...
  const variantFunction = (props: Props = {}) => {
    ...
    if (config.variants) {
      for (const [variantKey, variantValue] of Object.entries(resolvedProps)) {
        ...
        if (variantMap && variantValue !== undefined && variantValue !== false) {
          // Handle boolean values
          const valueKey = typeof variantValue === 'boolean' 
    ...
    if (config.compoundVariants) {
      for (const compoundVariant of config.compoundVariants) {
        ...
        const matches = Object.entries(conditions).every(([key, value]) => {
          ...
          if (typeof value === 'boolean' && typeof resolvedValue === 'boolean') {
